 ...
 */
export const toConnectedGroups = <T>(uf: UnionFind<T>) =>
    toGroups(uf).filter(lenGtOne)
...
export function unionFind<
    T = unknown,
    U extends T[] | number = number,
    V = U extends number ? number : T
>(
    itemsOrSize: U,
    map: (val: V) => number,
    ...
    const uf1: UnionFind<V> = {
        items,
        map,
        ranks: zeroesArray(items.length),
        roots: zeroesArray(items.length),
    }
 ...
 * @returns
 */
const replaceAt = <T>(list: T[], idx: number, val: T) => [
    ...list.slice(0, idx),
    val,
    ...list.slice(idx + 1),
]
 ...
 */
const incrementAt = (list: number[], idx: number) =>
    replaceAt(list, idx, list[idx] + 1)
 ...
 */
const rootOf = (roots: number[], item: number) =>
    roots[item] === 0 ? item : roots[item]
 ...
 */
export const find: Finder = <T extends { roots: number[] }>(
    { roots }: T,
    item: number
): number =>
...
export function link<T>(
    ...
    return {
        ...
        roots:
            ranks[gl] < ranks[gr]
                ? replaceTwo(roots, gr, gl, gl)
                : replaceTwo(roots, gr, gl, gr),
        ranks: ranks[gl] === ranks[gr] ? incrementAt(ranks, gr) : ranks,
    }
}

export const linkItem = <T>(uf: UnionFind<T>, left: T, right: T) =>
    link(uf, uf.map(left), uf.map(right))

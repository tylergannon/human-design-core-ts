...
export function buildPaths(): Array<ChartObject> {
    return flatten(
        channelData.map(channel => {
            return toArray(
                isCubicCurve(channel)
                    ? new CubicDraw(channel)
                    : isQuadraticCurve(channel)
                    ? new QuadraticDraw(channel)
...
abstract class ChannelDraw<T extends ChannelData> implements ChartObject {
    ...
    protected drawFull() {
        ...
        this.drawSide1()
        this.lineTo(add(this.data.point3Rel.full, this.data.arc2))
        this.drawSide3()
...
class QuadraticDraw extends ChannelDraw<QuadraticData> {
    copy() {
        return new QuadraticDraw(this.data)
    }
    drawSide1() {
...
class CubicDraw extends ChannelDraw<CubicData> {
    copy() {
        return new CubicDraw(this.data)
    }
    drawSide1(): void {
